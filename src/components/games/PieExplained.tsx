...
export default function PieExplained() {
  const [parts, setParts] = useState<[Part, Part, Part, Part, Part, Part]>([
    ...
    {
      baseClassName:
        "!fill-purple-800 stroke-5 transition-all stroke-stone-900",
      number: 0,
    },
    {
      baseClassName: "!fill-pink-800 stroke-5 transition-all stroke-stone-900",
  ...
  return (
    <div className="flex flex-col items-center **:cursor-pointer **:fill-pink-200 **:select-none">
      ...
      <div className="flex justify-center gap-5 !cursor-default **:!cursor-default">
        <div className="bg-pink-800 p-2 rounded-2xl text-pink-200">
          {parts[0].number} + {parts[2].number} + {parts[4].number} ={" "}
          <span className="font-bold text-white">
            {parts[0].number + parts[2].number + parts[4].number}
          </span>
        </div>
        <div className="p-2 px-4 bg-emerald-800 rounded-2xl text-emerald-200">
          2
        </div>
        <div className="bg-purple-800 p-2 rounded-2xl text-purple-200">
          {parts[1].number} + {parts[3].number} + {parts[5].number} ={" "}
          <span className="font-bold text-white">
            {parts[1].number + parts[3].number + parts[5].number}
          </span>
        </div>
